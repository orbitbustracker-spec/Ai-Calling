      ...
      <div className="flex justify-between items-end bg-slate-900/50 border border-white/10 rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl font-black text-white mb-2 flex items-center gap-2">
            <Building2 className="w-8 h-8 text-indigo-400" /> Organizations
          </h1>
          <p className="text-slate-400">Manage all tenant organizations on the platform.</p>
        </div>
        <CreateOrgModal />
      </div>
              ...
              {orgs.map(org => (
                <tr key={org.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="p-6 text-right space-x-3 flex items-center justify-end">
                    <Link href={`/admin/organizations/${org.id}`} className="inline-flex items-center gap-2 text-xs font-bold bg-indigo-500/10 text-indigo-400 px-3 py-1.5 rounded hover:bg-indigo-500/20 transition-colors">
                      <ShieldAlert className="w-3 h-3" /> Manage Sandbox
                    </Link>
                    <button className="text-slate-500 hover:text-indigo-400 transition-colors p-1"><Edit2 className="w-4 h-4" /></button>
