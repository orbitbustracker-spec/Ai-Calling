    ...
    <div className="h-full flex flex-col">
      <div className="flex gap-4 border-b border-slate-200 dark:border-white/10 pb-4 mb-6">
        <button 
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-colors ${activeTab === 'global-catalog' ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}
        >
          <Server className="w-4 h-4" /> Global Node Catalog
        </button>
        <button 
          onClick={() => setActiveTab('org-mapping')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition-colors ${activeTab === 'org-mapping' ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-300'}`}
        >
        ...
        {activeTab === 'global-catalog' && (
          <motion.div key="catalog" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">Master AI Catalog</h2>
                <p className="text-sm text-slate-500 dark:text-slate-400">Configure LLM, STT, and TTS engines once, then reuse them across clients.</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <Button onClick={() => setShowNodeModal(true)} className="bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white font-bold rounded-xl py-2 px-4 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
              {initialGlobalNodes.map(node => (
                <div key={node.id} className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl p-5 hover:border-indigo-500/30 transition-all group">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 dark:text-white">{node.providerName}</div>
                        <div className="text-[10px] text-slate-500 dark:text-slate-400 font-mono uppercase bg-gray-50 dark:bg-slate-950 px-2 py-0.5 rounded mt-1 inline-block">{node.category} ENGINE</div>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2 text-sm text-slate-500 dark:text-slate-400 font-mono">
          ...
          <motion.div key="mapping" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl p-6">
              ...
              <select 
                ...
                className="w-full max-w-md bg-gray-900/50 dark:bg-slate-950/80 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/50 font-bold"
              >
            ...
            {selectedOrgId && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/10 rounded-xl p-6">
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2"><Cpu className="w-5 h-5 text-indigo-400"/> Assign AI Pipeline</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Assigned LLM Engine</label>
                      <select value={orgPipeline.llm} onChange={e => setOrgPipeline({...orgPipeline, llm: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-lg px-4 py-2 text-slate-900 dark:text-white">
                        <option value="">-- Select LLM --</option>
                        {llmNodes.map(n => <option key={n.id} value={n.id}>{n.providerName}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Assigned STT Engine</label>
                      <select value={orgPipeline.stt} onChange={e => setOrgPipeline({...orgPipeline, stt: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-lg px-4 py-2 text-slate-900 dark:text-white">
                        <option value="">-- Select STT --</option>
                        {sttNodes.map(n => <option key={n.id} value={n.id}>{n.providerName}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">Assigned TTS Engine</label>
                      <select value={orgPipeline.tts} onChange={e => setOrgPipeline({...orgPipeline, tts: e.target.value})} className="w-full bg-gray-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-lg px-4 py-2 text-slate-900 dark:text-white">
                        <option value="">-- Select TTS --</option>
                        {ttsNodes.map(n => <option key={n.id} value={n.id}>{n.providerName}</option>)}
                      </select>
                    </div>
                    <Button onClick={handleSavePipeline} disabled={savingPipeline} className="w-full bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white font-bold py-2 rounded-lg mt-4">
      ...
      {showNodeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 dark:bg-slate-950/80 backdrop-blur-sm">
           <form onSubmit={handleCreateNode} className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-white/10 w-full max-w-lg shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <TerminalSquare className="w-5 h-5 text-indigo-400" /> Create Global AI Node
                </h3>
                <button type="button" onClick={()=>setShowNodeModal(false)} className="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white"><X className="w-5 h-5"/></button>
              </div>
              ...
              <div className="space-y-4">
                  <div>
                    ...
                    }} className="w-full bg-white dark:bg-slate-900 border border-indigo-500/30 text-indigo-400 rounded-lg px-4 py-2 text-sm focus:outline-none mb-4">
                  ...
                  <div>
                    ...
                    <input required value={nodeForm.name} onChange={e=>setNodeForm({...nodeForm, name: e.target.value})} type="text" placeholder="e.g. Groq LLM" className="w-full bg-gray-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 text-slate-900 dark:text-white focus:border-indigo-500 focus:outline-none" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input required value={nodeForm.endpoint} onChange={e=>setNodeForm({...nodeForm, endpoint: e.target.value})} type="url" placeholder="https://api..." className="w-full bg-gray-50 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-lg px-4 py-3 text-slate-900 dark:text-white font-mono focus:border-indigo-500 focus:outline-none" />
                  </div>
                  <div>
              ...
              <div className="flex gap-3 mt-8">
                <Button type="button" onClick={()=>setShowNodeModal(false)} className="flex-1 bg-transparent border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white">Cancel</Button>
